...
const nav = [
  ...
  { href: '/dashboard/events', label: 'Events', icon: ListIcon },
  { href: '/dashboard/prompts', label: 'Prompts', icon: SparklesIcon },
...
export function Sidebar({ session }: { session: Session }) {
  ...
  return (
    <aside className="w-56 flex-shrink-0 border-r border-white/10 flex flex-col bg-[#0d0d0d]">
      ...
      <div className="px-3 py-4 border-t border-white/10">
        <div className="flex items-center gap-2 px-3 py-2">
          {session.user?.image && (
            <Image src={session.user.image} alt={session.user.name ?? ''} width={28} height={28} className="rounded-full flex-shrink-0" />
          )}
          <div className="flex-1 min-w-0">
            <p className="text-white text-xs font-medium truncate">{session.user?.name}</p>
          </div>
          <button onClick={() => signOut({ callbackUrl: '/' })} className="text-gray-500 hover:text-white transition-colors flex-shrink-0" title="Sign out">
